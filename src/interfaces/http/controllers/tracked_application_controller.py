...
Reads, and one write — nothing that creates or destroys
------------------------------------------------------
There is deliberately no POST that creates a tracked application: a record
exists because an application was *sent*, so it is written by the flow that
sends one (`SubmitApplicationReview` via `SubmittedApplicationLog`). A route
that accepted a tracked application would let a caller assert that an employer
received a document, which is the one thing this table must never be able to
say untruthfully.
...
Every read carries the exact documents that went out, resolved from the ids
frozen onto the row at send time, so a tracker screen can name what an employer
received without a round trip per row.
