...
An insert whose foreign keys do not resolve is translated into
`TrackedApplicationReferenceError` rather than surfacing as a driver-level
integrity error. That failure means something filed an application against a
posting or a document snapshot that was never stored, and a tracker entry whose
documents point nowhere cannot do the one thing the tracker exists for — show
what was sent.
...
class SqlAlchemyTrackedApplicationRepository(TrackedApplicationRepository):
    ...
    def _apply_entity_to_model(
        ...
        # `job_posting_id`, `applied_at`, `resume_document_id`, and
        # `created_at` are deliberately not written back. They state what was
        # sent and when, which is not something a lifecycle update revises —
        # the domain has no operation that changes them either.
