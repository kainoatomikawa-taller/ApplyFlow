...
SENSITIVE: `AnswerApplicationFieldInput.value` is whatever the candidate typed
into a field on a real application form — routinely their address, their salary
expectation, or their EEO self-identification, since answering EEO themselves is
the *only* way it is ever answered. Never logged. Log the review session id and
the field id.

Sending is the candidate's act, and this is where that is enforced
-----------------------------------------------------------------
There is exactly one input type that can lead to a submission through a parked
session (`SubmitApplicationFormInput`), it names one review session, and it
carries the candidate's confirmation of every sensitive value ApplyFlow filled.
Nothing constructs one on a schedule, from a queue, or as a follow-on step of an
autofill pass: the only caller is the authenticated route the candidate hits when
they press Submit. "Nothing is submitted unattended" is that shape — a human
instruction is a required input, not a default.
"""
