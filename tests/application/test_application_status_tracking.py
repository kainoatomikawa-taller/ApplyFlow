...
def _documents() -> InMemoryApplicationDocumentRepository:
    """The snapshot every `_application()` above references.

    Seeded rather than left empty so these tests run against the same resolved
    path production takes — an empty store would exercise only the
    "reference does not resolve" branch, which is the rare one.
    """
...
async def test_a_status_update_moves_the_application_and_records_the_move() -> None:
    ...
    use_case = UpdateApplicationStatus(
        repository=repository, document_repository=_documents()
    )
...
async def test_updating_an_unknown_application_is_not_found() -> None:
    use_case = UpdateApplicationStatus(
        repository=_repository(), document_repository=_documents()
    )
...
async def test_reading_an_application_returns_its_history() -> None:
    ...
    use_case = GetTrackedApplication(
        repository=_repository(application), document_repository=_documents()
    )
...
async def test_the_feed_returns_a_candidates_applications_newest_first() -> None:
    ...
    use_case = ListTrackedApplications(
        repository=repository, document_repository=_documents()
    )
...
async def test_an_unknown_status_in_the_filter_is_refused() -> None:
    use_case = ListTrackedApplications(
        repository=_repository(_application()), document_repository=_documents()
    )
...
async def test_applications_to_one_posting_are_listed_together() -> None:
    ...
    use_case = ListApplicationsForJob(
        repository=repository, document_repository=_documents()
    )
